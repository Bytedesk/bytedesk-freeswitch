...
- [x] 默认增加支持开启 mod_shout 模块编译，并在 modules.conf.xml 中默认开启 mod_shout 模块支持
  - 已修复：Dockerfile 中将 mod_shout 的 sed 路径从错误的 `applications/mod_shout` 修正为 `formats/mod_shout`（上游 FreeSWITCH 1.10.12 build/modules.conf.in 中 mod_shout 实际位于 formats 目录）；modules.conf.xml 中已默认包含 `<load module="mod_shout" />`；依赖 libshout3-dev/libmpg123-dev/libvorbis-dev 已默认安装
